...
#include "slice.h"
namespace util {
const uint64_t kRAND64_MAX =
    ((((uint64_t)RAND_MAX) << 31) + ((uint64_t)RAND_MAX));
...
class TraceSeq : public Trace {
 public:
  explicit TraceSeq(uint64_t start_off = 0, uint64_t interval = 1,
                    uint64_t minimum = 0, uint64_t maximum = kRANDOM_RANGE)
      : Trace(0) {
...
class TraceUniform : public Trace {
 public:
  explicit TraceUniform(int seed, uint64_t minimum = 0,
...
#define FNV_PRIME_64 ((UINT64_C(1099511628211)))
  ...
  uint64_t Next() override {
    uint64_t d =
        (uint64_t)(-log(RandomDouble()) / gi_->gen.exponential.gamma) % range_;
    return d;
  }
...
class TraceNormal : public Trace {
 public:
  explicit TraceNormal(int seed, uint64_t minimum = 0,
                       uint64_t maximum = kRANDOM_RANGE)
      : Trace(seed) {
    gi_ = new GenInfo();
...
class TraceZipfian : public Trace {
 public:
  ...
  uint64_t Next() override {
    ...
    const uint64_t xz =
        gi_->gen.zipfian.min + (FNVHash64(z) % gi_->gen.zipfian.nr_items);
    return xz;
  }
  uint64_t NextRaw() {
  ...
  double ZetaRange(const uint64_t start, const uint64_t count,
                   const double theta) {
    double sum = 0.0;
    if (count > 0x10000000) {
